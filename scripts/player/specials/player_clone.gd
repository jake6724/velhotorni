...
@onready var clone_staff_sprite: StaffSprite = %StaffSprite
@onready var clone_staff_ap: AnimationPlayer = %StaffAnimationPlayer
@onready var spell_spawn_point: Node2D = %SpellSpawnPoint

func _ready():
	ap.play("idle")
	clone_staff_sprite.switch_staff_texture(player.player_spell_spawner.curr_spell_data.staff_type)
...
func rotate_staff() -> void:
	if player.player_aim.aim_input:
		...
		clone_staff_sprite.rotation = player.player_aim.aim_input.angle() + deg_to_rad(player.player_aim.staff_rotation_offset_degrees) * player.player_aim.staff_rotation_sign

		# Set staff render order based on aim direction and horizontal axis
		if player.player_aim.aim_input.normalized().y < 0:
			clone_staff_sprite.z_index = player.character_sprite.z_index - 1
		else:
			clone_staff_sprite.z_index = player.character_sprite.z_index + 1
...
func swing_staff() -> void:
	...
	var target = clone_staff_sprite.rotation_degrees + player.player_aim.SWING_DEGREE_INCREMENT
	tween.tween_property(clone_staff_sprite, "rotation_degrees", target, player.player_aim.SWING_ROTATION_SPEED)

func flip_sprite() -> void:
	if player.player_aim.aim_input:
		var flip = player.player_aim.aim_input.x <= -0.001
		clone_sprite.flip_h = flip
		clone_staff_sprite.flip_v = flip
...
func on_staff_switched(_spell_type: SpellData.StaffType) -> void:
	...
	clone_staff_sprite.switch_staff_texture(_spell_type)

func show_staff_sprite_custom(): 
	if player.staff_sprite.visible:
		clone_staff_sprite.show()
